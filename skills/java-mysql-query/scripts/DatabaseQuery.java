...
public class DatabaseQuery {
    ...
    public static void main(String[] args) {
        ...
        try (Connection conn = DriverManager.getConnection(url, dbUser, dbPassword)) {
            ...
            switch (command) {
                ...
                case "--table-deps":       handleTableDeps(conn, rest); break;
                case "--analyze-deep":     handleAnalyzeDeep(conn, rest); break;
    ...
    private static void printHelp() {
        ...
        String[][] cmds = {
            ...
            {"--explain <SQL>","获取SQL查询执行计划（EXPLAIN FORMAT=JSON）"},{"--export-csv <SQL> --output <文件>","导出查询结果到CSV文件"},{"--analyze-deep <表名>","深度表分析（含直方图、标准差、索引分析、分布桶）"},{"--table-deps","表依赖关系图（拓扑层级 + 环形依赖检测）"},{"--get-relations","外键关系拓扑 + Mermaid ERD"},
    ...
    // ========== 表依赖关系分析（--table-deps）==========
    private static void handleTableDeps(Connection conn, List<String> args) throws SQLException, IOException {
        String catalog = conn.getCatalog();
        StringBuilder sb = new StringBuilder();
        sb.append("{\"database\":\"").append(escapeJson(catalog)).append("\",\"relations\":[");
        ...
        try (ResultSet tables = conn.getMetaData().getTables(catalog, null, "%", new String[]{"TABLE"})) {
            while (tables.next()) {
                String tbl = tables.getString("TABLE_NAME");
                allTables.add(tbl);
                try (ResultSet fk = conn.getMetaData().getImportedKeys(catalog, null, tbl)) {
                    while (fk.next()) {
                        String parent = fk.getString("PKTABLE_NAME");
                        String child = fk.getString("FKTABLE_NAME");
                        if (!first) sb.append(",");
                        first = false;
                        sb.append("{\"parentTable\":\"").append(escapeJson(parent)).append("\",\"childTable\":\"").append(escapeJson(child)).append("\"}");
                        graph.computeIfAbsent(child, k -> new HashSet<>()).add(parent);
                        reverse.computeIfAbsent(parent, k -> new HashSet<>()).add(child);
        ...
        List<String> cycles = new java.util.ArrayList<>();
        for (String t : allTables) color.put(t, 0);
        for (String t : allTables) {
            if (color.get(t) == 0) { detectCycle(t, graph, color, path, cycles); }
        }
        ...
        for (String t : allTables) {
            ...
            firstLvl = false;
            sb.append("\"").append(escapeJson(t)).append("\":").append(levels.getOrDefault(t, -1));
        }
        sb.append("}");
        ...
        for (int i = 0; i < cycles.size(); i++) {
            if (i > 0) sb.append(",");
            sb.append("\"").append(escapeJson(cycles.get(i))).append("\"");
        }
        sb.append("]");

        sb.append(",\"maxLevel\":").append(level > 0 ? level - 1 : 0);
        sb.append(",\"tableCount\":").append(allTables.size());
        sb.append("}");
        System.out.print("{\"status\":\"table_deps_success\",\"deps\":" + sb.toString() + "}");
    }

    private static void detectCycle(String node, Map<String, Set<String>> graph, Map<String, Integer> color, List<String> path, List<String> cycles) {
        color.put(node, 1);
        path.add(node);
        for (String dep : graph.getOrDefault(node, new HashSet<>())) {
            ...
            } else if (c == 0) detectCycle(dep, graph, color, path, cycles);
        }
    ...
    // ========== 深度表分析（--analyze-deep）==========
    private static void handleAnalyzeDeep(Connection conn, List<String> args) throws SQLException {
        if (args.isEmpty()) { printError("请指定表名：--analyze-deep <表名>"); return; }
        String tableName = args.get(0);
        String catalog = conn.getCatalog();
        StringBuilder sb = new StringBuilder();
        sb.append("{\"table\":\"").append(escapeJson(tableName)).append("\"");
        ...
        try (Statement st = conn.createStatement();
             ResultSet rs = st.executeQuery(
                 "SELECT ENGINE, TABLE_ROWS, AVG_ROW_LENGTH, DATA_LENGTH, INDEX_LENGTH, AUTO_INCREMENT, " +
                 "TABLE_COMMENT, ROW_FORMAT, CREATE_TIME, UPDATE_TIME, TABLE_COLLATION " +
                 "FROM information_schema.TABLES WHERE TABLE_SCHEMA='" + escapeSql(catalog) + "' AND TABLE_NAME='" + escapeSql(tableName) + "'")) {
            if (rs.next()) {
                sb.append(",\"engine\":\"").append(escapeJson(nullToStr(rs.getString("ENGINE")))).append("\"");
                sb.append(",\"estimatedRows\":").append(rs.getLong("TABLE_ROWS"));
                sb.append(",\"avgRowLength\":").append(rs.getLong("AVG_ROW_LENGTH"));
                sb.append(",\"dataLengthBytes\":").append(rs.getLong("DATA_LENGTH"));
                sb.append(",\"indexLengthBytes\":").append(rs.getLong("INDEX_LENGTH"));
                sb.append(",\"autoIncrement\":").append(nullToStr(rs.getString("AUTO_INCREMENT")));
                sb.append(",\"rowFormat\":\"").append(escapeJson(nullToStr(rs.getString("ROW_FORMAT")))).append("\"");
                sb.append(",\"collation\":\"").append(escapeJson(nullToStr(rs.getString("TABLE_COLLATION")))).append("\"");
            }
        }
        ...
        try (ResultSet cols = meta.getColumns(catalog, null, tableName, "%")) {
            ...
            while (cols.next()) {
                ...
                sb.append(",\"size\":").append(cols.getInt("COLUMN_SIZE"));
                sb.append(",\"nullable\":").append("YES".equals(cols.getString("IS_NULLABLE")));
                analyzeDeepColumn(conn, tableName, colName, colType, sb);
                sb.append("}");
            }
        }
        sb.append("]");
        ...
        // 索引分析
        sb.append(",\"indexes\":[");
        try (ResultSet idx = meta.getIndexInfo(catalog, null, tableName, false, false)) {
            Map<String, IndexInfo> idxMap = new LinkedHashMap<>();
            while (idx.next()) {
                String idxName = idx.getString("INDEX_NAME");
                String colN = idx.getString("COLUMN_NAME");
                boolean nonUnique = idx.getBoolean("NON_UNIQUE");
                short type = idx.getShort("TYPE");
                if (idxName == null || colN == null) continue;
                IndexInfo ii = idxMap.computeIfAbsent(idxName, k -> new IndexInfo());
                ii.name = idxName;
                ii.unique = !nonUnique;
                ii.type = type;
                ii.cols.add(colN);
            ...
            boolean firstIdx = true;
            for (IndexInfo ii : idxMap.values()) {
                if (ii.name.equals("PRIMARY")) continue;
                if (!firstIdx) sb.append(",");
                firstIdx = false;
                sb.append("{\"name\":\"").append(escapeJson(ii.name)).append("\"");
                sb.append(",\"unique\":").append(ii.unique);
                sb.append(",\"columns\":[");
                for (int j = 0; j < ii.cols.size(); j++) {
                    if (j > 0) sb.append(",");
                    sb.append("\"").append(escapeJson(ii.cols.get(j))).append("\"");
                }
                sb.append("]");
                sb.append("}");
            }
        ...
        sb.append("}");
        System.out.print("{\"status\":\"analyze_deep_success\",\"analysis\":" + sb.toString() + "}");
    }
    ...
    private static void analyzeDeepColumn(Connection conn, String table, String column, String type, StringBuilder sb) {
        String safeTable = "`" + escapeSql(table) + "`";
        String safeCol = "`" + escapeSql(column) + "`";
        boolean isNumeric = type.matches("(?i)(INT|TINYINT|SMALLINT|MEDIUMINT|BIGINT|FLOAT|DOUBLE|DECIMAL|NUMERIC|REAL).*");
        boolean isString = type.matches("(?i)(CHAR|VARCHAR|TEXT|MEDIUMTEXT|LONGTEXT|ENUM|SET).*");
        ...
        try {
            ...
            try (Statement st = conn.createStatement();
                 ResultSet rs = st.executeQuery(
                     "SELECT COUNT(DISTINCT " + safeCol + ") AS dc, " +
                     "SUM(CASE WHEN " + safeCol + " IS NULL THEN 1 ELSE 0 END) AS nc, " +
                     "COUNT(*) AS tc FROM " + safeTable)) {
                if (rs.next()) {
                    long dc = rs.getLong("dc"), nc = rs.getLong("nc"), tc = rs.getLong("tc");
                    double nr = tc > 0 ? (double) nc / tc : 0;
                    sb.append(",\"distinctCount\":").append(dc);
                    sb.append(",\"nullCount\":").append(nc);
                    sb.append(",\"nullRatio\":").append(String.format("%.4f", nr));
                    sb.append(",\"distinctRatio\":").append(tc > 0 ? String.format("%.4f", (double) dc / tc) : "1.0");
                }
            }
            ...
            if (isNumeric) {
                try (Statement st = conn.createStatement();
                     ResultSet rs = st.executeQuery(
                         "SELECT MIN(" + safeCol + ") AS mn, MAX(" + safeCol + ") AS mx, " +
                         "AVG(" + safeCol + ") AS av, STDDEV(" + safeCol + ") AS sd FROM " + safeTable)) {
                    if (rs.next()) {
                        sb.append(",\"min\":\"").append(escapeJson(nullToStr(rs.getString("mn")))).append("\"");
                        sb.append(",\"max\":\"").append(escapeJson(nullToStr(rs.getString("mx")))).append("\"");
                        sb.append(",\"avg\":\"").append(escapeJson(nullToStr(rs.getString("av")))).append("\"");
                        sb.append(",\"stddev\":\"").append(escapeJson(nullToStr(rs.getString("sd")))).append("\"");
                    }
                }
                // 分布分桶
                try (Statement st = conn.createStatement();
                     ResultSet rs = st.executeQuery("SELECT COUNT(*) AS tc FROM " + safeTable + " WHERE " + safeCol + " IS NOT NULL")) {
                    if (rs.next()) {
                        long total = rs.getLong("tc");
                        if (total > 0) {
                            long step = Math.max(1, total / 10);
                            sb.append(",\"histogram\":[");
                            for (int b = 0; b < 10 && b * step < total; b++) {
                                if (b > 0) sb.append(",");
                                sb.append("{\"bucket\":").append(b);
                                sb.append(",\"offset\":").append(b * step).append("}");
                            }
                            sb.append("]");
                        }
                    }
            ...
            if (isString) {
                ...
                try (Statement st = conn.createStatement();
                     ResultSet rs = st.executeQuery(
                         "SELECT COUNT(*) AS emptyCnt FROM " + safeTable +
                         " WHERE " + safeCol + " = ''")) {
                    if (rs.next()) {
                        long emptyCnt = rs.getLong("emptyCnt");
                        sb.append(",\"emptyStringCount\":").append(emptyCnt);
                    }
                }
            }
            ...
            try (Statement st = conn.createStatement();
                 ResultSet rs = st.executeQuery(
                     "SELECT " + safeCol + " AS v, COUNT(*) AS c FROM " + safeTable +
                     " WHERE " + safeCol + " IS NOT NULL GROUP BY " + safeCol +
                ...
                while (rs.next()) {
                    if (!first) sb.append(",");
                    first = false;
                    sb.append("{\"v\":\"").append(escapeJson(nullToStr(rs.getString("v")))).append("\"");
                    sb.append(",\"c\":").append(rs.getLong("c")).append("}");
                }
                sb.append("]");
            }
